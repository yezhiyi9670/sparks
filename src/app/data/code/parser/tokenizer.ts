					...
					if(commentType == 'block') {
						if(ptr >= code.length) {
							...
							return {nextIndex: ptr, content: code.substring(index, ptr),
								issues: [createIssue(index, 'error', 'token.unclosed_comment')]
							}
			...
			reduce: (index, content, stringQuote: string) => {
				...
				if(ptr >= code.length) {
					return {nextIndex: ptr, content: content, extra: stringQuote,
						issues: [createIssue(index, 'error', 'token.unclosed_string')],
					finalize: true}
				}
				if(stringQuote == code[index]) {
					...
					if(typeChar == "\\" || typeChar == '"' || typeChar == "'") {
						...
						try {
							...
							return {nextIndex: ptr, content: content, extra: stringQuote,
								issues: [createIssue(index, 'error', 'token.invalid_escape', seq)]
							}
						}
						content += seq
					} else if(typeChar == 'u') {
